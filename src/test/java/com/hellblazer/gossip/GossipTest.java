...
public class GossipTest extends TestCase {
	...
	when(communications.handlerFor(isA(InetSocketAddress.class)))
		.thenReturn(handler);
	...
	ConcurrentMap<InetSocketAddress, Endpoint> endpoints = (ConcurrentMap<InetSocketAddress, Endpoint>) ep
		.get(gossip);
	...
	verify(gossipHandler).reply(
		asList(digest1a, digest2a, digest3a, digest4a),
	...
	gossip.examine(new Digest[] { digest1, digest2, digest3, digest4 },
		gossipHandler);
	verify(gossipHandler).reply(
		eq(asList(digest1a, digest3a)),
		eq(asList(new Update(address2, state2), new Update(address4,
			state4))));
