...
public class UdpCommunications implements GossipCommunications {
	...
	public void update(List<Update> deltaState) {
		...
		if (state.node.equals(gossipper)) {
			...
			log.trace(String
				.format("Not sending % to the node that owns it",
					state));
		    }
		} else {
    ...
    public static final Logger log = LoggerFactory
	    .getLogger(UdpCommunications.class);
	...
	try {
	    ...
	    throw new IllegalStateException(String.format(
		    "Unable to create default mac %s", DEFAULT_MAC_TYPE));
	} catch (InvalidKeyException e) {
	    throw new IllegalStateException(String.format(
		    "Invalid default key %s for default mac %s",
		    Arrays.toString(DEFAULT_KEY_DATA), DEFAULT_MAC_TYPE));
	}
	return mac;
    }

    public static String prettyPrint(SocketAddress sender,
	    SocketAddress target, byte[] bytes, int length) {
	...
	if (bound.getAddress().isAnyLocalAddress()) {
		...
		try {
		    ...
		    throw new IllegalStateException(
			    "Cannot determine local loopback address", e);
		}
	    } else {
    ...
    public UdpCommunications(InetSocketAddress endpoint,
	    ExecutorService executor, int receiveBufferMultiplier,
	    int sendBufferMultiplier, Mac mac, String networkInterface)
	    throws SocketException {
    ...
    public UdpCommunications(InetSocketAddress endpoint,
	    ExecutorService executor, Mac mac) throws SocketException {
	this(endpoint, executor, DEFAULT_RECEIVE_BUFFER_MULTIPLIER,
	...
	for (int i = 0; i < count; i++) {
		...
		if (log.isWarnEnabled()) {
		    log.warn(
			    String.format(
				    "Cannot deserialize digest. Ignoring the digest: %s\n%s",
				    i,
				    prettyPrint(sender, getLocalAddress(),
					    msg.array(), msg.limit())), e);
		}
		continue;
	    }
	    digests[i] = digest;
	...
	default: {
	    if (log.isInfoEnabled()) {
		log.info(format("invalid message type: %s from: %s", msgType,
			this));
	    }
	}
	}
	...
	if (log.isTraceEnabled()) {
	    log.trace(String.format(
		    "Received packet %s",
		    prettyPrint(packet.getSocketAddress(), getLocalAddress(),
			    buffer.array(), packet.getLength())));
	...
	dispatcher.execute(new Runnable() {
		...
		if (MAGIC == magic) {
			...
			if (!checkMac(buffer.array(), 0, packet.getLength()
			    ...
			    if (log.isWarnEnabled()) {
				log.warn(format(
					"Error processing inbound message on: %s, HMAC does not check",
					getLocalAddress()));
			    }
			    return;
			}
			...
			if (log.isWarnEnabled()) {
			    log.warn(
				    format("Error processing inbound message on: %s, HMAC does not check",
					    getLocalAddress()), e);
			}
			return;
		    }
			...
			if (log.isWarnEnabled()) {
			    log.warn(
				    format("Error processing inbound message on: %s",
					    getLocalAddress()), e);
			}
		    }
		} else {
		    if (log.isWarnEnabled()) {
			log.warn(format(
				"Msg with invalid MAGIC header [%s] discarded %s",
				magic,
				prettyPrint(packet.getSocketAddress(),
					getLocalAddress(), buffer.array(),
					packet.getLength())));
		    }
		}
		bufferPool.free(buffer);
	...
	return new Runnable() {
		...
		if (log.isInfoEnabled()) {
		    log.info(String.format(
			    "UDP Gossip communications started on %s",
			    localAddress));
		}
		while (running.get()) {
		    try {
