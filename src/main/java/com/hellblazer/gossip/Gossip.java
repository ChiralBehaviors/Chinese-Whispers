 ...
 */
public class Gossip {
	public static final UUID ALL_STATES = new UUID(0L, 0L);
	public static final UUID HEARTBEAT = new UUID(0L, 1L);
	...
	public Gossip(GossipCommunications communicationsService,
		...
		this(Generators.timeBasedGenerator(), communicationsService,
				systemView, failureDetectorFactory, random, gossipInterval,
				unit, DEFAULT_CLEANUP_CYCLES, DEFAULT_HEARTBEAT_CYCLE);
	}
				...
				.newSingleThreadScheduledExecutor(new ThreadFactory() {
					...
					public Thread newThread(Runnable r) {
						...
						Thread daemon = new Thread(r,
								"Gossip servicing thread " + count++);
						daemon.setDaemon(true);
						daemon.setUncaughtExceptionHandler(new UncaughtExceptionHandler() {
							@Override
							public void uncaughtException(Thread t, Throwable e) {
								log.warn(
										"Uncaught exception on the gossip servicing thread",
										e);
							}
						});
						return daemon;
		...
		dispatcher = Executors.newSingleThreadExecutor(new ThreadFactory() {
			...
			public Thread newThread(Runnable r) {
				...
				daemon.setUncaughtExceptionHandler(new UncaughtExceptionHandler() {
					...
					public void uncaughtException(Thread t, Throwable e) {
						log.warn(
								"Uncaught exception on the gossip dispatching thread",
								e);
					}
				});
				return daemon;
	...
	public void deregister(UUID id) {
		...
		ReplicatedState state = new ReplicatedState(id,
				System.currentTimeMillis(), EMPTY_STATE);
	...
	public UUID register(byte[] replicatedState) {
		...
		if (replicatedState.length > communications.getMaxStateSize()) {
			throw new IllegalArgumentException(String.format(
					"State size %s must not be > %s", replicatedState.length,
					communications.getMaxStateSize()));
		}
		UUID id = idGenerator.generate();
		ReplicatedState state = new ReplicatedState(id,
				System.currentTimeMillis(), replicatedState);
		synchronized (localState) {
			localState.put(id, state);
		}
		if (log.isDebugEnabled()) {
		...
		if (ALL_STATES.equals(digest.getId())) {
			...
			if (myState != null && myState.getTime() > digest.getTime()) {
				...
				log.trace(String.format(
						"Looking for deleteded local state %s on %s", digest,
						getLocalAddress()));
			}
		}
	}
	...
	private void updateHeartbeat() {
		...
		ReplicatedState heartbeat = new ReplicatedState(HEARTBEAT,
				System.currentTimeMillis(), EMPTY_STATE);
		synchronized (localState) {
	...
	protected void checkStatus() {
		...
		for (Iterator<Entry<InetSocketAddress, Endpoint>> iterator = endpoints
	...
	protected void discover(final InetSocketAddress address) {
		...
		final Endpoint endpoint = new Endpoint(address,
				communications.handlerFor(address));
				...
				public void run() {
					...
					if (log.isDebugEnabled()) {
						log.debug(String.format(
								"%s is now UP on %s (discover)", address,
								getLocalAddress()));
					}
					...
					// We want it all, baby
					endpoint.getHandler().gossip(
							Arrays.asList(new Digest(address, ALL_STATES, -1)));
				}
			}, fdFactory);
		}
	}
	...
	protected void examine(Digest[] digests, GossipMessages gossipHandler) {
		...
		for (Digest digest : digests) {
			if (ALL_STATES.equals(digest.getId())) {
				...
				Endpoint endpoint = endpoints.get(digest.getAddress());
				if (endpoint != null) {
					ReplicatedState localState = endpoint.getState(digest
							.getId());
					if (localState != null) {
						...
						deltaDigests.add(new Digest(digest.getAddress(), digest
								.getId(), -1L));
					}
				} else {
		...
		if (!deltaDigests.isEmpty() || !deltaState.isEmpty()) {
			if (log.isTraceEnabled()) {
				log.trace(String.format(
						"Member: %s replying with digests: %s state: %s",
						getLocalAddress(), deltaDigests, deltaState));
			}
	...
	protected void gossipWithTheDead(List<Digest> digests) {
		InetSocketAddress address = view.getRandomUnreachableMember(endpoints
				.size());
		if (address == null) {
			return;
		}
	...
	protected InetSocketAddress gossipWithTheLiving(List<Digest> digests) {
		...
		if (log.isWarnEnabled()) {
			log.warn(format(
					"Inconsistent state!  View thinks %s is alive, but service has no endpoint!",
					address));
		}
	...
	protected void notifyDeregister(final ReplicatedState state) {
		...
		if (log.isDebugEnabled()) {
			log.debug(String.format(
					"Member: %s notifying deregistration of: %s",
		...
		dispatcher.execute(new Runnable() {
			...
			public void run() {
				try {
					...
					log.warn(
							String.format(
									"exception notifying listener of deregistration of state %s",
									state.getId()), e);
				}
			}
		});
		...
		dispatcher.execute(new Runnable() {
			...
			public void run() {
				try {
					...
					log.warn(
							String.format(
									"exception notifying listener of registration of state %s",
									state.getId()), e);
				}
			}
		});
		...
		dispatcher.execute(new Runnable() {
			...
			public void run() {
				try {
					...
					log.warn(
							String.format(
									"exception notifying listener of update of state %s",
									state.getId()), e);
				}
			}
		});
	...
	protected boolean update(final Update update, InetSocketAddress gossiper) {
		...
		if (view.isQuarantined(update.node)) {
			if (log.isDebugEnabled()) {
				log.debug(format(
						"Ignoring gossip for %s because it is a quarantined endpoint",
						update));
			}
			return false;
		}
		...
		if (gossipingEndpoint == null) {
			...
			gossipingEndpoint.markAlive(new Runnable() {
				...
				public void run() {
					if (log.isDebugEnabled()) {
						log.debug(String.format(
								"%s is now UP on %s (check connect)", gossiper,
								getLocalAddress()));
					}
					...
					// We want it all, baby
					gossipingEndpoint.getHandler()
							.gossip(Arrays.asList(new Digest(gossiper,
									ALL_STATES, -1)));
				}
			}, fdFactory);
		}
